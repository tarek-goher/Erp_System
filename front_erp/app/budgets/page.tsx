...
// ══════════════════════════════════════════════════════════
// app/budgets/page.tsx — الميزانيات التقديرية (محدّثة)
// ══════════════════════════════════════════════════════════
...
export default function BudgetsPage() {
  ...
  // ✅ fmtDate معرّفة مرة واحدة بس هنا — مش مكررة في آخر الـ component
  const fmtDate = (d: string | null) =>
    d ? new Date(d).toLocaleDateString(lang === 'ar' ? 'ar-EG' : 'en-US') : '—'

  const [budgets,   setBudgets]   = useState<Budget[]>([])
  const [total,     setTotal]     = useState(0)
  const [lastPage,  setLastPage]  = useState(1)  // ✅ last_page من Laravel
  ...
  const fetchBudgets = async () => {
    setLoading(true)
    const res = await api.get<any>(`/budgets?page=${page}&per_page=15`)
    if (res.data) {
      const list = Array.isArray(res.data) ? res.data : (res.data.data || [])
      setBudgets(list)
      setTotal(res.data.total || list.length)
      setLastPage(res.data.last_page || 1)  // ✅ نحفظ last_page
    }
    setLoading(false)
  }
  ...
  const activeBudgets = budgets.filter(b => new Date(b.period_start) <= now && new Date(b.period_end) >= now)

  const openAdd = (e?: React.MouseEvent) => {
    if (e) { e.preventDefault(); e.stopPropagation() }
    setEditBudget(null)
    setForm({ ...EMPTY_FORM, period_start: now.toISOString().split('T')[0] })
    setFormErr('')
    setModal(true)
  }
  ...
  const openVs = async (b: Budget, e?: React.MouseEvent) => {
    ...
    setVsData(null)
    setVsModal(true)
    const res = await api.get<any>(`/budgets/${b.id}/vs`)
    if (res.data) {
      setVsData(res.data)
    } else {
      setVsData({ budget: b.amount, spent: 0, remaining: b.amount, percentage: 0 })
    }
    setVsLoading(false)
  }
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (new Date(form.period_end) <= new Date(form.period_start)) {
      setFormErr(ar('تاريخ النهاية يجب أن يكون بعد تاريخ البداية', 'End date must be after start date'))
      return
    }
  ...
  const periodStatus = (b: Budget) => {
    const start = new Date(b.period_start)
    const end   = new Date(b.period_end)
    if (now < start) return { label: ar('لم تبدأ', 'Not Started'), cls: 'badge-muted' }
    if (now > end)   return { label: ar('منتهية',   'Expired'),     cls: 'badge-warning' }
    return              { label: ar('نشطة',     'Active'),      cls: 'badge-success' }
  }
    ...
    <ERPLayout pageTitle={ar('الميزانيات', 'Budgets')}>
      ...
      {!loading && budgets.length > 0 && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px,1fr))', gap: '1rem', marginBottom: '1.25rem' }}>
          {[
            { label: ar('إجمالي الميزانيات', 'Total Budgets'), value: total,               icon: '📊' },
            { label: ar('إجمالي المبالغ',    'Total Amount'),  value: fmt(totalAmount),    icon: '💰' },
            { label: ar('الميزانيات النشطة', 'Active Now'),    value: activeBudgets.length, icon: '✅' },
          ].map(card => (
      ...
      <div className="card" style={{ padding: 0 }}>
        ...
        {/* ✅ Pagination صح — بنستخدم last_page من Laravel */}
        {total > 15 && (
      ...
      {vsModal && isMounted && createPortal(
        <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999999 }}
          onClick={() => { setVsModal(false); setVsData(null) }}>
          <div style={{ maxWidth: 480, width: '95%', background: 'var(--bg-card, #fff)', color: 'var(--text-color, #000)', borderRadius: 8, display: 'flex', flexDirection: 'column', maxHeight: '90vh', boxShadow: '0 20px 25px -5px rgba(0,0,0,0.2)' }}
                ...
                <>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem', marginBottom: '1rem' }}>
                    {[
                      ...
                      { label: ar('نسبة الصرف', 'Spent %'),  value: `${vsData.percentage ?? 0}%`, icon: '📈', color: (vsData.percentage ?? 0) > 100 ? 'var(--color-danger)' : 'var(--text-primary)' },
                    ].map(card => (
                  ...
                  <div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.35rem', fontSize: '0.85rem', color: 'var(--text-muted)' }}>
                      ...
                      <span>{vsData.percentage ?? 0}%</span>
                    </div>
                    <div style={{ height: 10, background: 'var(--bg-secondary)', borderRadius: 99, overflow: 'hidden' }}>
                      <div style={{
                        height: '100%',
                        width: `${Math.min(vsData.percentage ?? 0, 100)}%`,
                        background: (vsData.percentage ?? 0) > 100 ? 'var(--color-danger)' : (vsData.percentage ?? 0) > 80 ? 'var(--color-warning)' : 'var(--color-success)',
                        borderRadius: 99,
                        transition: 'width 0.5s ease',
                      }} />
                    </div>

                    {(vsData.percentage ?? 0) > 100 && (
                      <div style={{ color: 'var(--color-danger)', background: 'rgba(220,38,38,0.08)', padding: '0.625rem 0.875rem', borderRadius: 6, marginTop: '0.75rem', fontSize: '0.875rem', fontWeight: 600 }}>
                        🔴 {ar('تجاوزت الميزانية المحددة!', 'Budget exceeded!')}
                      </div>
                    )}
                    {(vsData.percentage ?? 0) > 80 && (vsData.percentage ?? 0) <= 100 && (
                      <div style={{ color: 'var(--color-warning)', background: 'rgba(234,179,8,0.08)', padding: '0.625rem 0.875rem', borderRadius: 6, marginTop: '0.75rem', fontSize: '0.875rem', fontWeight: 600 }}>
                        🟡 {ar('اقتربت من حد الميزانية (80%+)', 'Approaching budget limit (80%+)')}
                      </div>
                    )}
                  </div>

                  {vsBudget && (
                    <div style={{ marginTop: '1rem', background: 'var(--bg-secondary)', borderRadius: 6, padding: '0.75rem 1rem', fontSize: '0.875rem', color: 'var(--text-muted)' }}>
                      <p style={{ margin: 0 }}>📅 {fmtDate(vsBudget.period_start)} → {fmtDate(vsBudget.period_end)}</p>
                      {vsBudget.notes && <p style={{ margin: '0.3rem 0 0' }}>📝 {vsBudget.notes}</p>}
                    </div>
                  )}
                </>
              ) : (
      ...
      {modal && isMounted && createPortal(
        <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999999 }}
          ...
          <div style={{ maxWidth: 580, width: '95%', background: 'var(--bg-card, #fff)', color: 'var(--text-color, #000)', borderRadius: 8, display: 'flex', flexDirection: 'column', maxHeight: '90vh', boxShadow: '0 20px 25px -5px rgba(0,0,0,0.2)' }}
            onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              <h3 className="modal-title">
              ...
              <div className="modal-body" style={{ overflowY: 'auto' }}>
                <div className="form-grid form-grid-2">
                  <div className="input-group" style={{ gridColumn: '1 / -1' }}>
                    ...
                    <input className="input" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} placeholder={ar('مثال: ميزانية المبيعات 2025', 'e.g. Sales Budget 2025')} autoFocus />
                  </div>
      ...
      {deleteId && isMounted && createPortal(
        <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999999 }}
          ...
          <div style={{ maxWidth: 400, width: '95%', background: 'var(--bg-card, #fff)', color: 'var(--text-color, #000)', borderRadius: 8, boxShadow: '0 20px 25px -5px rgba(0,0,0,0.2)' }}
            ...
            <div className="modal-body">
              <p>{ar('هل أنت متأكد من حذف هذه الميزانية؟', 'Are you sure you want to delete this budget?')}</p>
            </div>
            <div className="modal-footer">
