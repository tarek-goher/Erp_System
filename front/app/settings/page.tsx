    ...
    return (
      <ERPLayout pageTitle={t('settings')}>
        <div className="tabs">
          ...
          {lang === 'ar' ? tabLabels[tab]?.ar ?? tab : tabLabels[tab]?.en ?? tab}  
            </button>
          ))}
        </div>
  ...
  setEditing(cat)
  setForm({ name: cat.name || '', type: cat.type || 'product', description: cat.description || '' })
  setShowForm(true) 
}
    ...
    const handleSubmit = async (e: FormEvent) => {
      ...
      setSaving(false)
      console.log('holiday error details:', res)
...
{/* <div className="input-group">
  ...
  function HolidaysSettings({ lang, t }: any) {
    ...
    const [saving, setSaving] = useState(false)
    const [editing, setEditing] = useState<any>(null)
   const [form, setForm] = useState({ name: '', holiday_date: '', description: '' })
    const [companyId, setCompanyId] = useState<number | null>(null)

    useEffect(() => {
      const fetchCompany = async () => {
      const res = await api.get('/company')
  const id = res.data?.data?.id || res.data?.id || 1
        setCompanyId(id)
    ...
    const openAdd = () => { setEditing(null); setForm({ name: '', holiday_date: '', description: '' }); setModal(true) }
    const openEdit = (h: any) => { setEditing(h);setForm({ name: h.name, holiday_date: h.holiday_date, description: h.description || '' }); setModal(true) }
                ...
                <div className="modal-body">
                  <div className="input-grid">
                    ...
                    <div className="input-group">
                      ...
                      <input className="input" type="date" value={form.holiday_date} onChange={e => setForm({ ...form, holiday_date: e.target.value })} required />
                    </div>
                    <div className="input-group" style={{ gridColumn: '1 / -1' }}>
