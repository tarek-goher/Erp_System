...
export default function UsersPage() {
  ...
  useEffect(() => {
    api.get('/roles').then(r => {
      ...
      // ضمان وجود branch_manager دايماً حتى لو مش موجود في الـ DB
      const REQUIRED_ROLES = ['branch_manager']
      let maxId = mapped.reduce((m, r) => Math.max(m, r.id || 0), 0)
      REQUIRED_ROLES.forEach(roleName => {
        if (!mapped.find(r => r.name === roleName)) {
          mapped.push({ id: ++maxId, name: roleName })
        }
      })
      setRoles(mapped)
    })
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    // Branch manager: automatically assign users to own branch
    if (isBranchManager && (currentUser as any)?.branch_id) {
      payload.branch_id = (currentUser as any).branch_id
    }
  ...
  return (
    <ERPLayout pageTitle={ar ? 'المستخدمون' : 'Users'}>
      {isBranchManager && (
        <div style={{ background:'var(--color-warning-light)', border:'1px solid var(--color-warning)', borderRadius:'var(--radius-md)', padding:'0.6rem 1rem', marginBottom:'1rem', fontSize:'0.85rem', color:'var(--color-warning-dark, #92400e)' }}>
          🏪 {ar ? 'أنت تدير مستخدمي فرعك فقط — سيتم تعيين الفرع تلقائياً عند إضافة مستخدم جديد.' : 'You manage only your branch users — branch will be assigned automatically.'}
        </div>
      )}
