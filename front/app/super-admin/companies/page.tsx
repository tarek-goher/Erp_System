...
// ✅ أضفنا admin_email منفصل عن إيميل الشركة
const EMPTY_FORM = { company_name: '', name: '', email: '', admin_email: '', phone: '', plan: 'starter', country: 'مصر', password: '' }
...
export default function CompaniesPage() {
  ...
  const handleAdd = async () => {
    ...
    // ✅ إيميل الأدمن لازم يكون مختلف عن إيميل الشركة
    if (!form.admin_email) {
      show('إيميل المدير مطلوب', 'error'); return
    }
    if (form.admin_email === form.email) {
      show('إيميل المدير لازم يكون مختلف عن إيميل الشركة', 'error'); return
    }
    setSaving(true)
    const res = await api.post('/super-admin/companies', {
      name:               form.company_name,
      admin_name:         form.name,
      email:              form.email,         // إيميل الشركة
      admin_email:        form.admin_email,   // ✅ إيميل الأدمن منفصل
      phone:              form.phone,
    ...
    show(`تم إضافة الشركة ${form.company_name} ✅`)
  ...
  const handleDelete = async () => {
    if (!deleteId) return
   const res = await api.delete(`/super-admin/companies/${deleteId}`)
    if (res.error) { show(res.error, 'error'); return }
  ...
  const handleImpersonate = async (company: Company) => {
    ...
    if (token) {
      const targetUrl = url || `/dashboard?impersonate_token=${token}`
      window.open(targetUrl, '_blank')
      show(`✅ تم الدخول كـ ${company.name}`)
    } else {
      show('لم يُرجع السيرفر توكن الـ impersonate', 'error')
    }
  }
  ...
  const handleImport = async () => {
    ...
    for (const row of importRows) {
      ...
      const res = await api.post('/super-admin/companies', body)
  ...
  return (
    <div>
                    ...
                    <td>
                      <div style={{ display: 'flex', gap: 6 }}>
                        ...
                        <button
                          ...
                          title="دخول كـ tenant هذه الشركة"
                        >
                          {impersonating === c.id ? '⏳' : '🔑 دخول'}
                        </button>
      ...
      {/* ✅ Modal إضافة شركة جديدة مع إيميل منفصل للأدمن */}
      <Modal open={showAdd} onClose={() => setShowAdd(false)} title="إضافة شركة جديدة" size="md"
        ...
        <div className="form-grid form-grid-2">
          ...
          <div className="input-group"><label className="input-label">إيميل الشركة *</label>
            <input style={INP} type="email" value={form.email} onChange={e => setForm(p => ({ ...p, email: e.target.value }))} placeholder="info@company.com" dir="ltr" /></div>
          {/* ✅ إيميل المدير منفصل */}
          <div className="input-group"><label className="input-label">إيميل المدير *</label>
            <input style={INP} type="email" value={form.admin_email} onChange={e => setForm(p => ({ ...p, admin_email: e.target.value }))} placeholder="admin@company.com" dir="ltr" /></div>
